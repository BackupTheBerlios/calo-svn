...
FullItemDU::FullItemDU (Item *theItem)
{
	_item = theItem;
	_play.clear();
...
void
FullItemDU::layout (const Cairo::RefPtr<Cairo::Context>& cr)
{
	if (_item->_article.empty())
		return;
...
/// plus space).
void 
FullItemDU::render (const Cairo::RefPtr<Cairo::Context>& cctx, double x, double y)
{
	if (_item->_article.empty())
		return;
#ifdef DEBUG
